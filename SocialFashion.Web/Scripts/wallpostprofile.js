...
function Post(data, hub) {
    ...
    self.CountLike = ko.observable(data.CountLike);
    ...
    self.addComment = function () {
        self.hub.server.addComment({ "StatusId": self.PostId, "Comment": self.newCommentMessage() }).done(function (comment) {
    ...
    self.addLike = function () {
        self.hub.server.addLike({ "StatusId": self.PostId }).done(function (countLike) {
...
function viewModel() {
    ...
    self.init = function () {
        self.error(null);
        self.hub.server.getPostProfile().fail(function (err) {
            self.error(err);
        });
    }
    ...
    self.addPost = function () {
        self.error(null);
        self.hub.server.addPost({ "Content": self.newMessage(), "Privacy": self.sltStatus() }).fail(function (err) {
            self.error(err);
        });
    }
    ...
    self.hub.client.loadPosts = function (data, userId) {
    ...
    self.hub.client.newPost = function (post, b, userId) {
        if (b == 0) {
            self.newPosts.splice(0, 0, new Post(post, self.hub));
        }
        else if (b == 1) {
            self.hub.server.isFriend(self.userId(), userId).done(function (bCheck) {
                if (bCheck) {
                    self.newPosts.splice(0, 0, new Post(post, self.hub));
                }
            });
    ...
    self.hub.client.newLike = function (countLike, postId) {
        //check in existing posts
        var posts = $.grep(self.posts(), function (item) {
            return item.PostId === postId;
        });
        posts[0].CountLike(countLike);
    }
